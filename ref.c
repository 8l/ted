...
extern int	getline2 P_((char *, int, FILE *));
...
# endif
	...
	/* search the file */
	for (comment = -1L, thislnum = 0; here = ftell(sfile), thislnum++, getline2(buf, BLKSIZE, sfile) > 0; )
	{
		...
		if (lnum == thislnum || (entry && !strncmp(buf, entry, len)))
		{
			...
			if (!noargs)
			{
				...
				while (getline2(buf, BLKSIZE, sfile) > 0
				    && buf[0] != '#'
				    && strchr(buf, '{') == (char *)0)
				{
					fputs(buf, stdout);
